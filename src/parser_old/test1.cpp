...
public:
    virtual void run(const MatchFinder::MatchResult &result) {
        const CXXRecordDecl* cxxDecl = result.Nodes.getNodeAs<clang::CXXRecordDecl>("class");
        if(!cxxDecl->hasDefinition() ||
            cxxDecl->isCXXClassMember()) {
            // return if not have definition or is class member
            return;
        }
        cxxDecl->dump();
        std::cerr<<"Definition: "<<cxxDecl->hasDefinition()<<"\n";
        std::cerr<<"Is Member: "<<cxxDecl->isCXXClassMember()<<"\n";
        for(auto it = cxxDecl->field_begin();it!=cxxDecl->field_end();it++) {
            std::cerr<<"\tname: "<<it->getNameAsString()<<"\n";
            std::cerr<<"\ttype: "<<it->getType().getAsString()<<"\n";
        }
        std::cerr<<"\n";
        for(auto it = cxxDecl->method_begin(); it!=cxxDecl->method_end(); it++) {
            std::cerr<<"\tname: "<<it->getNameInfo().getName().getAsString()<<"\n";
        }
        const clang::ASTRecordLayout& layout(cxxDecl->getASTContext().getASTRecordLayout(cxxDecl));
        std::cerr<<"size: "<<layout.getSize().getQuantity()<<"\n";
        std::cerr<<"alignment: "<<layout.getAlignment().getQuantity()<<"\n";
    }
};
...
public:
    virtual void run(const MatchFinder::MatchResult &Result) {
        ...
        std::cerr<<"IsVariadic: "<<funcDecl->isVariadic()<<"\n";
        std::cerr<<"CXX method: "<<funcDecl->isCXXClassMember()<<"\n"; 
...
int main(int argc, const char **argv) {
    ...
    DeclarationMatcher classMatcher = cxxRecordDecl().bind("class");
    FuncPrinter printer;
    ClassPrinter class_printer;
    MatchFinder finder;
    finder.addMatcher(funcMatcher, &printer);
    finder.addMatcher(classMatcher, &class_printer);
